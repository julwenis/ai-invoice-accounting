...
def parse_with_gemini(file, api_key, max_retries=1):
    ...
    try:
        ...
        return {"_dosya": file.name, "Nr Faktury": "Hata", "Not": f"Görsel okunamadı: {e}", "_time": 0.0}
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            parsed["_time"] = round(time.perf_counter() - start_time, 2)
    ...
    return {
        ...
        "_time": round(time.perf_counter() - start_time, 2)
    }
...
def create_professional_excel(data_list):
    ...
    headers = [
        "Lp.", "Data Wystawienia", "Data Sprzedaży", "Nr Faktury",
        "Satıcı Unvanı", "NIP Sprzedawcy", "NIP Nabywcy", "Netto (PLN)",
        "Stawka VAT", "Kwota VAT (PLN)", "Wartość Brutto (PLN)",
        "Sposób Płatności", "Uwaga / Kontrola"
    ]
    ws.append(headers)
    ...
    for idx, data in enumerate(data_list, 1):
        ws.append([
            idx, data.get("Data Wystawienia", ""), data.get("Data Sprzedaży", ""),
            data.get("Nr Faktury", ""), data.get("Satıcı Unvanı", ""),
            data.get("NIP Sprzedawcy", ""), data.get("NIP Nabywcy", ""),
            data.get("Netto (PLN)", 0.0), data.get("Stawka VAT", ""),
            data.get("Kwota VAT (PLN)", 0.0), data.get("Wartość Brutto (PLN)", 0.0),
            data.get("Sposób Płatności", ""), data.get("Not", "")
        ])
